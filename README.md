# mermaid2aichat-dsh — Mermaid 可视化编辑器（DeepSeek Harness 插件）

把 Mermaid 反向编辑器迁移为 DeepSeek Harness（dsh）的浏览器插件：
...
- **响应式紧凑模式**：编辑器宽度不足（< 420px）时自动隐藏左右侧面板，只显示画布；
  头部「▥ 紧凑」按钮可随时手动切换
...
- 四种图表类型切换：**代码区下拉选择器**（流程图/时序图/类图/ER图）或代码首行修改，
  弹窗确认；流程图方向与连线模式（按方向/就近）选择器同在代码区
...
- 独立暗色模式（标题栏明暗切换，仅作用于面板内部，不影响 dsh 主题）
...
### 与 dsh-web-ui（aionui-panel）共存

dsh-web-ui 的右侧文件树/预览面板使用同一类网格追加方案，本插件的布局控制器
实现了与它的协作协议，两者可同时显示：

- 检测到 aionui 的面板列时，写入 6 轨 = shell 3 轨 + 编辑器轨 + 其预览轨 +
  其文件树轨；编辑器列固定在对方列之前，保持对方拖拽把手的宽度假设不变；
- 对方写入 5 轨时同帧读取其两轨数值并恢复 6 轨（瞬时覆盖不可见）；
- 对方列已存在但数值未知时写一次纯 shell 3 轨「推一把」，借对方的写入
  收敛到 6 轨（覆盖对方的 HMR / 后挂载时序）；
- 内置写入爆发护栏：若第三方网格写入者导致观察器级联成环，自动冷却
  200ms 断开循环，避免页面卡死。
