...
- **聊天区让位 + 可调宽度**：编辑器打开时自动占用右侧 details 列，聊天区自动缩宽、
  不遮挡对话内容；左侧边缘把手可拖动调整编辑器宽度（与 details 列同步）
...
- 聊天区让位复用 dsh 的 details 列（产品布局只对**非空白会话**打开该列）：
  空白会话下编辑器浮于聊天区上方；编辑器关闭时会一并收起 details 列
  （若用户同时打开了会话详情，会被一同关闭）。

### 宽度上限与宿主补丁

details 列宽度受 dsh 布局契约限制（默认上限 520px，且聊天区保留 640px 下限）。
本插件在宿主布局服务开放 `setDetails` 时**自绘拖拽把手**直接驱动列宽；未开放时
降级为穿透原生把手（300–520px）。要获得更宽的编辑器（最高 960px），需要对 dsh
应用以下小补丁（改动 `packages/client/ui-layout` 并重建其 client bundle）：

- `src/client/service.ts`：`ILayout` 与 `LayoutController` 增加 `setDetails(px)`；
- `src/client/columns.ts`：`DETAILS_MAX` 520 → 960，`CENTER_MIN` 640 → 480。

不应用补丁插件照常工作，仅宽度上限为 520px。
